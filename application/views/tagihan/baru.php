              ...
              <div class="row">
                <div class="col-lg-4">
                  <div class="form-group">
                    ...
                    <select name="idMasuk" id="idMasuk" class="custom-select form-control">
                      <option disabled selected>Pilih Nomor Pembelian</option>
                      <?php foreach ($barang_masuk as $masuk) : 
                        $total = $masuk['subtotal'];
                        $ppn = 0.1;
                        $hitung_ppn = $total * $ppn;
                        $subtotal = $hitung_ppn + $total;
                      ?>
                        <option id="masuk-<?= $masuk['id'] ?>" value="<?= $masuk['id'] ?>" 
                                data-tanggal="<?= $masuk['tgl_masuk'] ?>"
                                data-supplier="<?= $masuk['nama_supplier'] ?>"
                                data-subtotal="<?= $subtotal ?>"
                        ><?= '['.$masuk['kode_masuk']. ' - '. $masuk['nama_supplier'].']' ?></option>
                      <?php endforeach ?>
                    </select>
                  </div>
                </div>
                ...
                <div class="col-lg-4">
                  <div class="form-group">
                    ...
                    <select name="idRetur" id="idRetur" class="custom-select form-control">
                      <option disabled selected>Pilih Nomor Retur</option>
                      <?php foreach ($barang_retur as $retur) : 
                        $total = $retur['subtotal'];
                        $ppn = 0.1;
                        $hitung_ppn = $total * $ppn;
                        $subtotal = $hitung_ppn + $total;
                      ?>
                        <option id="retur-<?= $retur['kode_retur'] ?>" value="<?= $retur['kode_retur'] ?>" 
                                data-totalretur ="<?= $subtotal ?>"
                        ><?= '['.$retur['kode_retur']. ' - '. $retur['nama_supplier'].']' ?></option>
                      <?php endforeach ?>
                    </select>
                  </div>
                </div>                
                ...
                <div class="col-lg-4">
                  <div class="form-group">
                    ...
                    <input type="text" name="totalretur" id="totalretur" class="form-control" value="0" readonly>
                  </div>
                </div>                
...
$(document).ready(function() {
  ...
  $(document).on('change', '#idMasuk', function() {
      var getTanggal = $("#masuk-"+this.value).data('tanggal');
      var getSupplier = $("#masuk-"+this.value).data('supplier');
      var getSubtotal = $("#masuk-"+this.value).data('subtotal');
      $("#tanggal").val(getTanggal);
      $("#supplier").val(getSupplier);
      $("#subtotal").val(getSubtotal);
  });

  $(document).on('change', '#idRetur', function() {
      var getTotalRetur = $("#retur-"+this.value).data('totalretur');
      $("#totalretur").val(getTotalRetur);
  });  
