          ...
          <div class="card-body">
            <div class="card-title">
              <div class="text-center">
                <h3>
                Edit Barang Retur
                <a href="<?= site_url('barang-retur') ?>" class="btn btn-sm btn-secondary float-right"><i class="fa fa-arrow-circle-right"></i> Kembali</a>
                </h3>
                <hr>
              </div>
            </div>

            <form id="barangretur-form" role="form">
              <div class="row">
                <div class="col-lg-4">
                  <div class="form-group">
                    <label>Tanggal Retur</label>
                    <input type="hidden" name="id_barang_retur"  id="id_barang_retur" value="<?= $edit_barang_retur['id'] ?>">                    
                    <input type="date" name="tgl_retur" class="form-control" value="<?= $edit_barang_retur['tgl_retur'] ?>">
                  </div>
                </div>
                ...
                <div class="col-lg-8">
                  <div class="form-group">
                    ...
                    <select name="supplier" class="form-control">
                      ...
                      <?php if ($sup['id'] == $edit_barang_retur['id_supplier']) : ?>
...
$(document).ready(function() {
  $(document).on('change', '#id_barang', function() {
    ...
    function loadBarang(){
        $.ajax({
            url: "<?= site_url('BarangRetur/getBarangReturDetail/' . $this->uri->segment(3)) ?>",
            method: 'GET',
            dataType: 'JSON',
            beforeSend: function(){
            ...
            success: function(response){
                ...
                $.each(response.data,function(k, v){
                    ...
                    html += '<tr>'
                            + '<td>'
                            +   '<div class="form-group">'
                            +       '<input type="hidden" class="form-control" id="id_barang_retur_detail" name="id_barang_retur_detail" placeholder="" readonly value="'+v.id_barang_retur_detail+'">'
                    ...
                    html += '<td>'
                            +   '<div class="form-group">'
                            +       '<input type="text" class="form-control" id="qty" name="qty" placeholder="Qty" value="'+v.jumlah_retur+'">'
                            +   '</div>'
                            +'</td>';

                    html += '<td>'
                            +   '<div class="form-group">'
                            +       '<input type="text" class="form-control" id="keterangan" name="keterangan" placeholder="Keterangan" value="'+v.keterangan+'">'
                            +       '</div>'
                            +   '</div>'
                            +'</td>';                            
                    ...
                    html += '<td>'
                            +   '<div class="form-group">'
                            +       '<button type="button" class="btn btn-sm btn-danger" onclick="deleteBarang('+v.id_barang_retur_detail+')"><i class="fa fa-trash"></i>'
                            +       '</button>'
                            +   '</div>'
                            +'</td>';
  ...
  function updateBarang(id){
    ...
    var keterangan = $(id).closest('tr').find('#keterangan').val();
    var id_barang_retur_detail = $(id).closest('tr').find('#id_barang_retur_detail').val();
    $.ajax({
        url: "<?= site_url('BarangRetur/updateBarangReturDetail/') ?>" + id_barang_retur_detail,
        method: 'post',
        data: {
        qty: qty,
        id_barang: id_barang,
  ...
  function deleteQty(id){
    ...
    var id_barang_retur_detail = $(id).closest('tr').find('#id_barang_retur_detail').val();
      $.ajax({
          url: "<?= site_url('BarangRetur/deleteQtyBarang/') ?>" + id_barang_retur_detail,
          method: 'post',
          data: {
          qty: qty,
          id_barang: id_barang
    ...
    function createBarang(id)
    {
        ...
        var qty = $(id).closest('tr').find('#qty').val();
        var keterangan = $(id).closest('tr').find('#keterangan').val();
        var id_barang_retur = $('#id_barang_retur').val();
        $.ajax({
          url: '<?= site_url('BarangRetur/createBarangReturDetail') ?>',
          method: 'post',
          data: {
            id_barang:id_barang,
            qty: qty,
            id_barang_retur: id_barang_retur,
            keterangan: keterangan
          },
          success: function(response){
            loadBarang();
    ...
    {
        $.ajax({
          url: '<?= site_url('BarangRetur/deleteBarangReturDetail/') ?>' + id,
          method: 'post',
          success: function(response){
            loadBarang();
  ...
  $('#barangretur-form').submit(function(e){
    e.preventDefault();
    $.ajax({
      method: "POST",
      url: "<?= site_url('BarangRetur/update/').$this->uri->segment(3) ?>",
      data: $(this).serialize(),
      dataType: "JSON",
      beforeSend: function(){
      ...
      success: function(data){
        ...
        if (data.status == "success") {
          ...
          window.location.href="<?= site_url('BarangRetur'); ?>";
        } else {
